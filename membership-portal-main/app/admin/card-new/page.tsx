...
function NewCardDesignerPageContent() {
  ...
  */
  ...
  async function printCard() {
     ...
     * This does NOT use window.print(), so Android Chrome cannot produce
     * an empty/white print preview. The existing card HTML is rendered
     * directly to a CR80 PDF: 85.6mm × 53.9mm.
     */
    const existing = document.getElementById("pdf-render-root");
    existing?.remove();
    ...
    root.innerHTML = `
      <style>
        #pdf-render-root .print-card-page {
          position: relative; width: ${CARD_WIDTH}px; height: ${CARD_HEIGHT}px;
          overflow: hidden; background: #fff; page-break-after: always;
        }
        #pdf-render-root .print-card-inner {
          position: relative; width: ${CARD_WIDTH}px; height: ${CARD_HEIGHT}px;
          overflow: hidden; box-sizing: border-box;
          border: 4px solid #075c2b; border-radius: 22px;
          background: linear-gradient(135deg,#ffffff 0%,#f7fff8 55%,#e8f6df 100%);
        }
        #pdf-render-root .print-card-background {
          position:absolute; inset:0;
          background:linear-gradient(135deg,#ffffff 0%,#f7fff8 55%,#e8f6df 100%);
        }
        #pdf-render-root .print-card-top {
          position:absolute; left:0; right:0; top:0; height:115px;
          background:linear-gradient(135deg,#ffffff,#f6fff8); border-bottom:10px solid #075c2b;
        }
        #pdf-render-root .print-card-bottom {
          position:absolute; left:0; right:0; bottom:0; height:150px;
          background:linear-gradient(to top,#d8efc8,transparent); opacity:.55;
        }
        #pdf-render-root img { display:block; }
      </style>
    ...
    try {
      ...
      const pages = Array.from(
        root.querySelectorAll<HTMLElement>(".print-card-page")
      );

      if (pages.length !== 2) {
        throw new Error("Front/Back card pages could not be prepared.");
      }
      ...
      for (let index = 0; index < pages.length; index++) {
        ...
        if (index > 0) {
          pdf.addPage([85.6, 53.9], "landscape");
        }

        pdf.addImage(image, "PNG", 0, 0, 85.6, 53.9, undefined, "FAST");
      }

      const membership = String(
        firstValue(memberData, [
          "membership_number", "membership_no", "member_number",
          "member_no", "membership_id", "card_number"
        ]) || "member"
      ).replace(/[^a-zA-Z0-9_-]+/g, "-");

      pdf.save(`membership-card-${membership}.pdf`);
    } catch (error: any) {
