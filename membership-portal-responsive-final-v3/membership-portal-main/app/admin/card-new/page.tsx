...
function NewCardDesignerPageContent() {
  ...
  */
  ...
  async function printCard() {
    ...
    // IMPORTANT: html2canvas must capture an element that is actually
    // rendered inside the browser viewport. Putting it far off-screen or
    // behind the page can produce a completely blank canvas in mobile Chrome.
    ...
    style.textContent = `
      #pdf-capture-container .print-card-page {
        position: relative; width: ${CARD_WIDTH}px; height: ${CARD_HEIGHT}px;
        overflow: hidden; background: #fff; box-sizing: border-box;
      }
      #pdf-capture-container .print-card-inner {
        position: absolute; left: 0; top: 0; width: ${CARD_WIDTH}px; height: ${CARD_HEIGHT}px;
        overflow: hidden; box-sizing: border-box; border: 4px solid #075c2b;
        border-radius: 22px; background: linear-gradient(135deg,#ffffff 0%,#f7fff8 55%,#e8f6df 100%);
      }
      #pdf-capture-container .print-card-background { position:absolute; inset:0; background:linear-gradient(135deg,#ffffff 0%,#f7fff8 55%,#e8f6df 100%); }
      #pdf-capture-container .print-card-top { position:absolute; left:0; right:0; top:0; height:72px; background:#075c2b; }
      #pdf-capture-container .print-card-bottom { position:absolute; left:0; right:0; bottom:0; height:38px; background:#075c2b; }
      #pdf-capture-container img { display:block; }
    `;
    container.appendChild(style);
    container.insertAdjacentHTML("beforeend", `${buildPrintCardHtml("front")}${buildPrintCardHtml("back")}`);
    ...
    try {
      ...
      const pages = Array.from(container.querySelectorAll<HTMLElement>(".print-card-page"));
      if (pages.length < 2) throw new Error("Front/Back card render ಆಗಲಿಲ್ಲ.");

      // Force layout before capture. This is especially important on
      // Android Chrome where a newly inserted fixed element can otherwise
      // report zero/incorrect layout during the first frame.
      pages.forEach((page) => {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        if (i > 0) pdf.addPage([85.6, 53.9], "landscape");
        pdf.addImage(canvas.toDataURL("image/png", 1), "PNG", 0, 0, 85.6, 53.9, undefined, "FAST");
      }

      const memberNo = String(memberData?.membership_no || memberData?.membershipNumber || "card").replace(/[^a-zA-Z0-9_-]/g, "_");
      pdf.save(`Membership-Card-${memberNo}.pdf`);
    } catch (error: any) {
